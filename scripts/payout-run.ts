...
async function buildPlan() {
  ...
  return { perUserAmount, count, transfers: resolved, unresolved };
}
...
async function main() {
  ...
  if (plan.unresolved.length > 0) {
    console.error(`Unresolved users: ${plan.unresolved.length}. Resolve addresses before sending.`);
  }
  const results = [] as any[];
  for (const t of plan.transfers) {
    const res = await client.send(t.toAddress, t.amount, `rank ${t.rank}`);
    results.push(res);
  }
